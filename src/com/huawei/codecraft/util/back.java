...
//    double rx,ry;   // 距离矢量
    ...
    double wallCoef = 3.2;  // 靠墙判定系数，多大算靠墙
    double avoidCoef = 1.5;  // 靠墙判定系数，多大算靠墙
...
//            }
    ...
    private void processEmergEvent() {
        ...
        if (Main.clockCoef[robot.id] != -1) {   // 判断过了，被前面的机器人置为了 -1
            ...
            Point vec2 = 
            double vec = speed1.x * speed2.y - speed1.y * speed2.x;
...
//    }
    ...
    public void calcParamEveryFrame() {
        ...
        calcClockwise();    // 转动方向
        calcMinDistance();
        // calcAvoidRanking(); // 避让优先级
    }

    // 获取避让优先级
    public void calcAvoidRanking() {
        // 离目标越近，避让付出代价越大,暂时设定距离是1.6-1.8m
        robot.avoidRank =  Robot.avoidMin;
        if (realDistance < avoidCoef * robot.getRadius()) {
            robot.avoidRank =   Robot.avoidMin + 1 + robot.carry > 0 ? 1 : 0;
        }
    }
    ...
    public boolean isNotInEdge() {
        if (robot.pos.x - wallCoef * robot.getRadius() < 0 || robot.pos.y - wallCoef * robot.getRadius() < 0 || robot.pos.x + wallCoef * robot.getRadius() - 50 > 0 || robot.pos.y + wallCoef * robot.getRadius() - 50 > 0) {
            return false;
        }
        return true;
    }
